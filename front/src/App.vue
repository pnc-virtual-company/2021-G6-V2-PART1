...
export default {
  ...
  methods: {
    ...
    SignIn(loginInfo){
      ...
      .then(response =>{
        if(response.data.message1 === "Email does not match!"){
          this.errorSignin.email_error = response.data.message1 + ' Please try again';
        }else if(response.data.message2 === "Password does not match!"){
          this.errorSignin.password_error = response.data.message2 + ' Please try again';
        }else{
           this.errorSignin.password_error = response.data.message2;
        }
